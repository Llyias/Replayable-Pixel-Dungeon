...
public enum HeroClass {
	...
	private static void giveDebugItems() {
		new CMD().quantity(100).collect();
		collectDebugLeveledItem( new RingOfHaste(), 100 );
		collectDebugLeveledItem( new RingOfMight(), 100 );
		collectDebugLeveledItem( new RingOfTenacity(), 100 );

		WandOfBlastWave blastWave = new WandOfBlastWave();
		blastWave.identify();
		blastWave.curCharges = blastWave.maxCharges;
		blastWave.collect();

		new PotionOfMindVision().identify();
		new PotionOfMindVision().quantity( 10 ).collect();
	}

	private static void collectDebugLeveledItem( Item item, int level ) {
		item.identify();
		item.level( level );
		item.collect();
	}
